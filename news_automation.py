...
class NewsKeyword(BaseModel):
    ...
    source_type: str = "keyword"  # keyword or url
    description: Optional[str] = None
    is_active: bool = True
    update_frequency: int = 10  # minutes
    hours_back: int = 8
...
def init_db():
    with get_connection() as conn:
        ...
        CREATE TABLE IF NOT EXISTS newsfeeds (
            ...
            keywords TEXT NOT NULL, -- JSON serialized list
...
# Utility for stable id from URL (for articles guid)
...
# CRUD for Articles (list only - no create here, articles created by collection)
...
# Public feed endpoint: serves RSS or JSON

def escape_rss_text(text: str) -> str:
...
def get_public_feed(feed_id: int):
    ...
    ET.SubElement(channel, "description").text = f"RSS feed generated for keywords: {', '.join(feed['keywords'])}"
    ET.SubElement(channel, "language").text = "pt-br"

    for art in articles:
        item = ET.SubElement(channel, "item")
        ET.SubElement(item, "title").text = escape_rss_text(art["title"])
        ET.SubElement(item, "link").text = art.get("url", "")
        ET.SubElement(item, "guid").text = stable_id(art.get("url", art["title"]))
        ET.SubElement(item, "pubDate").text = art.get("published_date", "").replace("T", " ") if art.get("published_date") else ""
        ET.SubElement(item, "description").text = escape_rss_text(art.get("summary") or art.get("content") or "")
        ET.SubElement(item, "source").text = art.get("source") or ""
        ET.SubElement(item, "category").text = art.get("keyword") or ""

    xml_str = ET.tostring(rss, encoding="utf-8")
    return Response(content=xml_str, media_type="application/rss+xml")

# Simulated news collection function (replace this with your LLM or scraper)
...
def collect_news(keyword_id: int):
    with get_connection() as conn:
        ...
        # Simulated articles collection - replace with your scraping or LLM calls
        sample_articles = [
            {
                "title": f"Notícia sobre {kw['keyword']} em {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                "content": f"Conteúdo gerado sobre a palavra-chave {kw['keyword']}.",
        ...
        for art in sample_articles:
            conn.execute(
                """INSERT INTO newsarticles (title, content, summary, source, url, published_date, keyword, relevance_score, category)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (art["title"], art["content"], art["summary"], art["source"], art["url"],
                 art["published_date"], art["keyword"], art["relevance_score"], art["category"])
            )
        conn.execute(
            "UPDATE newskeywords SET last_updated = ? WHERE id = ?",
